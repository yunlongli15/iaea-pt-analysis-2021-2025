...
def create_year_specific_plots(year_dfs):
    ...
    for df, file_count, year in year_dfs:
        ...
        title_text = f'Laboratory Performance Analysis - {year} (Filtered)\n{stats_text}'
        if special_info:
        ...
        # Save image
        output_file = f"{output_folder}/filtered_labcode_a_count_{year}.png"
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Chart saved: {output_file}")
        
        # Save detailed data to CSV
        csv_file = f"{output_folder}/filtered_labcode_a_count_{year}.csv"
...
def create_comparison_plot(year_dfs):
    ...
    ax2.set_title('Average A Count by Year (Filtered)', fontsize=14, fontweight='bold')
    ...
    ax3.set_title('Maximum A Count by Year (Filtered)', fontsize=14, fontweight='bold')
    ...
    filter_note = "Filtered: Skipped Tl-208, Pb-210, Bi-212, Pb-212, Bi-214, Pb-214, Ac-228, Pa-234m, Th234 with Gamma_Soil/Vegetation"
    ax4.set_title(f'Special Laboratory Performance\n{filter_note}', fontsize=14, fontweight='bold', pad=20)
    
    plt.tight_layout()
    
    # Save comparison plot
    output_folder = "filtered_labcode_analysis_charts"
    output_file = f"{output_folder}/filtered_labcode_a_count_comparison.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Comparison plot saved: {output_file}")
    
    # Save comparison data
    comp_csv = f"{output_folder}/filtered_labcode_a_count_comparison.csv"
...
def create_special_labs_trend(year_dfs):
    ...
    ax1.set_title('Special Labs: A Count Trend (Filtered)', fontsize=14, fontweight='bold')
    ...
    # Save trend chart
    output_folder = "filtered_labcode_analysis_charts"
    output_file = f"{output_folder}/filtered_special_labs_a_trend.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    """
    print("="*70)
    print("Laboratory A-Count Performance Analysis (2021-2025) - FILTERED VERSION")
    print("="*70)
