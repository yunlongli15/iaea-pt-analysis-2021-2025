...
def analyze_single_year(year, special_lab=None):
    """
    Analyze laboratory participation for a single year
    """
    ...
    # 定义需要跳过的特定放射性核素
    skip_nuclides = [
        'Tl-208_Gamma_Soil', 'Pb-210_Gamma_Soil', 'Bi-212_Gamma_Soil', 'Pb-212_Gamma_Soil', 
        'Bi-214_Gamma_Soil', 'Pb-214_Gamma_Soil', 'Ac-228_Gamma_Soil', 'Pa-234m_Gamma_Soil', 'Th-234_Gamma_Soil',
        'Bi-214_Gamma_Vegetation', 'Pb-214_Gamma_Vegetation'
    ]
    
    print(f"{year}: Found {len(all_files)} files initially")
    
    # 筛选文件：跳过包含特定核素和Gamma_Soil或Gamma_Vegetation后缀的文件
    filtered_files = []
    for file_path in all_files:
        file_name = os.path.basename(file_path)
        skip_file = False
        
        # 检查文件名中是否包含特定核素和Gamma后缀的组合
        for nuclide in skip_nuclides:
            if (f"{nuclide}" in file_name or 
                f"{nuclide}" in file_name):
                skip_file = True
                break
    ...
    print(f"{year}: After filtering, {len(filtered_files)} files remain for analysis")
    
    if not filtered_files:
        print(f"{year}: No files remaining after filtering")
        return None, None, 0
    
    # Count laboratory participation
    ...
    for file_path in filtered_files:
        try:
            ...
            if labcode_col is not None:
                # Count laboratories in this file
    ...
    # Convert to DataFrame and sort by participation count
    if lab_participation:
        df = pd.DataFrame(list(lab_participation.items()), 
                         columns=['labcode', f'count_{year}'])
        df = df.sort_values(f'count_{year}', ascending=False)
        
        # 先排序
        df = df.sort_values(f'count_{year}', ascending=False).reset_index(drop=True)
        ...
        return df, len(filtered_files), year
    else:
        return None, 0, year
...
def create_year_specific_plots(year_dfs):
    ...
    """
    # Create output folder - 使用新的文件夹名称
    output_folder = "filtered_labcode_analysis_charts"
    ...
    for df, file_count, year in year_dfs:
        ...
        # Plot scatter points
        plt.scatter(df['x'], df[f'count_{year}'], 
                   s=df['size'], 
                   c=df['color'], 
                   alpha=0.7, 
                   edgecolors='black', 
        ...
        plt.ylabel(f'Number of Analysis Projects ({year})', fontsize=12, fontweight='bold')
        ...
        for _, row in special_labs.iterrows():
            ...
            text_offset = 15 if row[f'count_{year}'] < y_max * 0.8 else -15
            
            plt.annotate(f"{row['labcode']}",
                        xy=(row['x'], row[f'count_{year}']),
        ...
        total_labs = len(df)
        avg_participation = df[f'count_{year}'].mean()
        max_participation = df[f'count_{year}'].max()
        
        # Get special laboratory information
        special_info = ""
        if not special_labs.empty:
            for _, row in special_labs.iterrows():
                special_info += f"{row['labcode']}: Rank {row[f'rank_{year}']}, {row[f'count_{year}']} projects; "
        
        stats_text = f"Total Labs: {total_labs} | Avg Participation: {avg_participation:.1f} | Max: {max_participation}"
        title_text = f'Laboratory Participation Analysis - {year} (Filtered)\n{stats_text}'
        if special_info:
            title_text += f"\nSpecial Labs: {special_info}"
        
        # 添加过滤说明
        filter_note = "Filtered: Skipped Tl-208, Pb-210, Bi-212, Pb-212, Bi-214, Pb-214, Ac-228, Pa-234m, Th234 with Gamma_Soil/Vegetation"
        full_title = f'{title_text}\n{filter_note}'
        
        plt.title(full_title, fontsize=14, fontweight='bold', pad=25)
        ...
        # Save image
        output_file = f"{output_folder}/filtered_labcode_participation_{year}.png"
        plt.savefig(output_file, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Chart saved: {output_file}")
        
        # Save detailed data to CSV
        csv_file = f"{output_folder}/filtered_labcode_participation_{year}.csv"
...
def create_comparison_plot(year_dfs):
    ...
    for df, file_count, year in year_dfs:
        if df is not None and len(df) > 0:
            ...
            avg_participation = df[f'count_{year}'].mean()
            max_participation = df[f'count_{year}'].max()
            min_participation = df[f'count_{year}'].min()
            ...
            if not special_labs.empty:
                for _, row in special_labs.iterrows():
                    special_info += f"{row['labcode']}({row[f'count_{year}']}) "
            
            comparison_data.append({
                'Year': year,
                'Total Projects': file_count,
                'Total Labs': total_labs,
                'Avg Participation': avg_participation,
    ...
    ax1.set_title('Projects and Labs by Year (Filtered)', fontsize=14, fontweight='bold')
    ...
    # Chart 2: Average participation
    ax2 = axes[0, 1]
    bars3 = ax2.bar(comp_df['Year'], comp_df['Avg Participation'], 
                   alpha=0.7, color='orange')
    
    ax2.set_xlabel('Year', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Avg Projects per Lab', fontsize=12, fontweight='bold')
    ax2.set_title('Average Lab Participation by Year (Filtered)', fontsize=14, fontweight='bold')
    ...
    # Chart 3: Maximum participation
    ax3 = axes[1, 0]
    bars4 = ax3.bar(comp_df['Year'], comp_df['Max Participation'], 
                   alpha=0.7, color='green')
    
    ax3.set_xlabel('Year', fontsize=12, fontweight='bold')
    ax3.set_ylabel('Max Projects', fontsize=12, fontweight='bold')
    ax3.set_title('Maximum Participation by Year (Filtered)', fontsize=14, fontweight='bold')
    ...
    table = ax4.table(cellText=table_data,
                     colLabels=['Year', 'Special Labs (Projects)'],
                     cellLoc='left',
                     loc='center',
                     colWidths=[0.2, 0.8])
    ...
    filter_note = "Filtered: Skipped Tl-208, Pb-210, Bi-212, Pb-212, Bi-214, Pb-214, Ac-228, Pa-234m, Th234 with Gamma_Soil/Vegetation"
    ax4.set_title(f'Special Laboratory Information\n{filter_note}', fontsize=14, fontweight='bold', pad=20)
    
    plt.tight_layout()
    
    # Save comparison plot
    output_folder = "filtered_labcode_analysis_charts"
    output_file = f"{output_folder}/filtered_labcode_participation_comparison.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Comparison plot saved: {output_file}")
    
    # Save comparison data
    comp_csv = f"{output_folder}/filtered_labcode_participation_comparison.csv"
...
def create_special_labs_trend(year_dfs):
        ...
        if df is not None and year in special_labs_by_year:
            ...
            if not special_row.empty:
                ...
                trend_data.append({
                    'Year': year,
                    'Labcode': row['labcode'],
                    'Participation': row[f'count_{year}'],
    ...
    participations = trend_df['Participation']
    
    colors = ['red', 'green', 'blue', 'orange']
    
    for i, (year, labcode, participation, color) in enumerate(zip(years, labcodes, participations, colors)):
        ax1.bar(i, participation, color=color, alpha=0.7, edgecolor='black')
        ax1.text(i, participation + 0.5, f'{labcode}', 
                ha='center', va='bottom', fontsize=12, fontweight='bold')
    
    ax1.set_xlabel('Year', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Number of Projects', fontsize=12, fontweight='bold')
    ax1.set_title('Special Labs: Project Participation (Filtered)', fontsize=14, fontweight='bold')
    ...
    for i, participation in enumerate(participations):
        ax1.text(i, participation/2, f'{int(participation)}', 
    ...
    for i, (year, labcode, rank) in enumerate(zip(years, labcodes, ranks)):
        ax2.text(i, rank + 0.5, f'{labcode}: Rank {int(rank)}', 
                ha='center', va='bottom', fontsize=10)
    
    ax2.set_xlabel('Year', fontsize=12, fontweight='bold')
    ax2.set_ylabel('Rank', fontsize=12, fontweight='bold')
    ax2.set_title('Special Labs: Ranking Trend (Filtered)', fontsize=14, fontweight='bold')
    ...
    # 添加过滤说明
    filter_note = "Filtered: Skipped Tl-208, Pb-210, Bi-212, Pb-212, Bi-214, Pb-214, Ac-228, Pa-234m, Th234 with Gamma_Soil/Vegetation"
    fig.suptitle(filter_note, fontsize=10, y=0.02)
    
    plt.tight_layout()
    
    # Save trend chart
    output_folder = "filtered_labcode_analysis_charts"
    output_file = f"{output_folder}/filtered_special_labs_trend.png"
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    """
    Main function: Analyze laboratory participation for 2021-2024
    """
    print("="*70)
    print("Laboratory Participation Analysis (2021-2025) - FILTERED VERSION")
    print("="*70)
    ...
    print("  2025: Lab 15")
    print("\nFiltering Criteria:")
    print("  Skipping files containing: Tl-208, Pb-210, Bi-212, Pb-212, Bi-214,")
    print("  Pb-214, Ac-228, Pa-234m, Th234 with _Gamma_Soil or _Gamma_Vegetation")
    print("="*70)
    ...
    for year in years:
        ...
        if df is not None:
            ...
            print(f"  Avg Projects per Lab: {df[f'count_{year_val}'].mean():.1f}")
            print(f"  Max Projects: {df[f'count_{year_val}'].max()}")
            ...
            if not special.empty:
                ...
                for _, row in special.iterrows():
                    ...
                    count = row[f'count_{year_val}']
                    percentage = (count / file_count) * 100
                    print(f"    {row['labcode']}: Rank {rank}, {count} projects ({percentage:.1f}%)")
    ...
    print(f"All charts saved to 'filtered_labcode_analysis_charts' folder")
    print(f"Filtered files containing: Tl-208, Pb-210, Bi-212, Pb-212, Bi-214,")
    print(f"Pb-214, Ac-228, Pa-234m, Th234 with _Gamma_Soil or _Gamma_Vegetation")
    print(f"{'='*70}")
